...
private:
  ...
  // ***************************************************
  void insert(const Robot& robot, Robot*& aNode); //helper function for insert
  void inOrder(Robot* aBot) const ; //lists nodes in ascendinf id order
  int height(Robot* aBot); //helper function to set height
  bool findBot(int id, Robot* aBot)const; //returns true if node exists
  Robot* rightRotation(Robot* aBot); //R
  Robot* leftRotation(Robot* aBot); //L
  Robot* getBot(int id, Robot* aBot); //returns pointer to node
  Robot* remove(int id, Robot* aBot); //helper function for remove
  Robot* findMin(Robot* aBot); 
  Robot* removeDead(Robot* aBot);
